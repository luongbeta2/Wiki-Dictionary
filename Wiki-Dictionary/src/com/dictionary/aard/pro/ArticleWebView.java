...
import com.dictionary.activity.ArticleCollectionActivity;

public class ArticleWebView extends WebView {

	public static final String LOCALHOST = Application.LOCALHOST;
	public final String styleSwitcherJs;

	String TAG = getClass().getSimpleName();

	public final static String PREF = "articleView";
	public static final String PREF_TEXT_ZOOM = "textZoom";
	public static final String PREF_STYLE = "style.";
	public static final String PREF_REMOTE_CONTENT = "remoteContent";
	...
	public ArticleWebView(Context context, AttributeSet attrs) {
		...
		this.setWebViewClient(new WebViewClient() {
			...
			public boolean shouldOverrideUrlLoading(WebView view, final String url) {
				Log.d(TAG, String.format("shouldOverrideUrlLoading: %s (current %s)", url, view.getUrl()));

				Uri uri = Uri.parse(url);
				String scheme = uri.getScheme();
				String host = uri.getHost();

				if (externalSchemes.contains(scheme) || (scheme.equals("http") && !host.equals(LOCALHOST))) {
	...
	public void setStyle(String styleTitle) {
		...
		if (prefs.contains(styleTitle)) {
			...
			js = String.format("javascript:" + Application.jsClearUserStyle + Application.jsSetCannedStyle, getCurrentSlobId(), styleTitle);
		}
		if (Log.isLoggable(TAG, Log.DEBUG)) {
			Log.d(TAG, js);
		}
	...
	public  void beforeLoadUrl(String url) {
		setCurrentSlobIdFromUrl(url);
		updateBackgrounColor();
	}

	public  void updateBackgrounColor() {
		...
		if (preferredStyle != null) {
			...
			// load started and/or before page's style applies (and even after
			// that if
			// style doesn't explicitly set background).
			// this is a hack to preemptively set "right" background and prevent
			// extra flash
			//
			// TODO Hack it even more - allow style title to include background
			// color spec
