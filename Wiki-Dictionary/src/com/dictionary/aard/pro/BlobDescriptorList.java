...
public final class BlobDescriptorList extends AbstractList<BlobDescriptor> {
	...
	public void notifyDataSetChanged() {
		...
		if (filter == null || filter.length() == 0) {
			...
			for (BlobDescriptor bd : this.list) {
				StringSearch stringSearch = new StringSearch(filter, new StringCharacterIterator(bd.key), filterCollator);
	...
	private void sortOrderChanged() {
		try {
			...
			/*
			 * Synopsis: Updated sort behavior for Arrays and Collections may
			 * throw an IllegalArgumentException Description: The sorting
			 * algorithm used by java.util.Arrays.sort and (indirectly) by
			 * java.util.Collections.sort has been replaced. The new sort
			 * implementation may throw an IllegalArgumentException if it
			 * detects a Comparable that violates the Comparable contract. The
			 * previous implementation silently ignored such a situation. If the
			 * previous behavior is desired, you can use the new system
			 * property, java.util.Arrays.useLegacyMergeSort, to restore
			 * previous mergesort behavior. Nature of Incompatibility:
			 * behavioral RFE: 6804124
			 */
			// Name comparators use ICU collation key comparison. Given Unicode
			// collation complexity
			// it's hard to be sure that collation key comparisons won't trigger
			// an exception. It certainly
	...
	public Slob.Blob resolve(BlobDescriptor bd) {
		...
		if (slobId.equals(bd.slobId)) {
			...
			try {
				...
				Log.w(TAG, String.format("Failed to resolve descriptor %s (%s) in %s (%s)", bd.blobId, bd.key, slob.getId(),
						slob.file.getAbsolutePath()), ex);
				blob = null;
			}
		}
		if (blob != null) {
